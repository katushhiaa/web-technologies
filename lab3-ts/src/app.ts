...
function render() {
  ...
  svc.books.getAll().forEach((b) => {
    ...
    const status = b.borrowedBy
      ? `<span class="badge bg-warning text-dark me-3">Позичено</span>`
      : "";
    li.innerHTML = `
      <div>
    ...
    li.querySelector<HTMLButtonElement>(".action-btn")!.onclick = async () => {
      try {
        if (!b.borrowedBy) {
          ...
          if (!userId) return;
          if (!digitsOnly.test(userId)) {
            await modals.info("Помилка", "ID має містити лише цифри.");
            return;
          }

          const uid = String(userId);
          const current = svc.books.getAll().filter((x) => String(x.borrowedBy) === uid).length;
          console.log("[limit-check]", {
            userId: uid,
            current,
            max: DomainService.MAX_BORROWED,
          });

          if (current >= DomainService.MAX_BORROWED) {
            const msg = `Ліміт: користувач може мати не більше ${DomainService.MAX_BORROWED}-х книг.`;
            try {
              await modals.info("Не вдалося", msg);
            } catch {
              alert(msg);
            }
            return;
          ...
          const res = svc.borrowBook(b.id, uid);
          if (res === "OK") {
            const u = svc.users.getById(uid)!;
            await modals.info(
              "Успіх",
            ...
            try {
              await modals.info("Не вдалося", res);
            } catch {
              alert(res);
            }
          }
        ...
        console.error(err);
        try {
          await modals.info("Помилка", "Сталася неочікувана помилка. Спробуйте ще раз.");
        } catch {
